...
Page magic:
    b"SENS" -> IMU records, 40 bytes each; bytes 17..20 continuous lux, 21..38 reserved
    b"AUD0" -> PCM int16 audio
    b"LITE" -> one AS7341 session result, 44-byte payload (includes session average lux)
"""
...
# Updated to 44 bytes to account for the float and 3-byte struct padding
LIGHT_RESULT_PAYLOAD_SIZE = 44 
LIGHT_RESULT_STRUCT_FORMAT = "<10H4IB3sf"
...
LIGHT_RESULT_COLUMNS = [
    ...
    "light_level_class",
    "light_level_label",
    "session_average_lux",  # Added field for the session average
    "page_sequence",
    "page_timestamp_ms",
    "physical_page_index",
    "page_version",
...
def parse_sensor_record(record: bytes) -> dict:
    ...
    # Extract continuous lux float (4 bytes)
    continuous_lux = struct.unpack("<f", record[17:21])[0]

    # Verify the remaining reserved section is still zeroed out
    legacy_light_reserved = record[21:39]
    legacy_light_reserved_all_zero = all(byte == 0 for byte in legacy_light_reserved)
    if not legacy_light_reserved_all_zero:
        print("Warning: SENS legacy light reserved bytes 21..38 are not all zero.")
...
def parse_light_result_payload(payload: bytes) -> dict:
    ...
    if reserved_padding != b"\x00\x00\x00":
        print(f"Warning: LITE struct padding bytes are not zero: {reserved_padding!r}")
    ...
    parsed.update(
        {
            ...
            "session_average_lux": calculated_lux,
        }
    )
...
def parse_nand_dump(bin_filename: Path, imu_csv_filename: Path, light_csv_filename: Path, wav_filename: Path,
    ...
    if not light_df.empty:
        ...
        summary += (
            ...
            f"Last session lux average: {last_light['session_average_lux']:.2f}\n"
...
def plot_continuous_lux(df: pd.DataFrame, output_prefix: Path) -> None:
    if df.empty or "continuous_lux" not in df.columns:
        print("No continuous Lux data available; skipping plot.")
        return
    ...
    plt.plot(t, df["continuous_lux"], label="Illuminance", color="#FFA500")
    plt.xlabel(x_label)
    plt.ylabel("Illuminance [Lux]")
    plt.title("Continuous Real-Time Lux")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    lux_png = output_prefix.with_name(output_prefix.name + "_continuous_lux.png")
    plt.savefig(lux_png, dpi=200)
...
def plot_light_results(light_df: pd.DataFrame, output_prefix: Path) -> None:
    ...
    for row_index, (_, row) in enumerate(light_df.iterrows(), start=1):
        ...
        title = (
            ...
            f"Avg Lux: {row['session_average_lux']:.2f} | "
...
def main():
    ...
    try:
        ...
        plot_continuous_lux(imu_df, output_prefix)
